...
def get_embedding(face_img):
    """קבלת PIL Image בגודל 128x128, מחזיר embedding מנורמל"""
    ...
    return embedding  # כבר מנורמל מ-l2_norm
    ...
    try:
        ...
        for face_obj in face_objs:
            if face_obj["confidence"] < confidence_threshold:
                continue
            # face_obj["face"] הוא numpy array מנורמל [0,1]
            face_arr = face_obj["face"]
            face_pil = Image.fromarray((face_arr * 255).astype(np.uint8)).convert("RGB")
            # שמירת ה-box לציור מסגרות
...
def recognize_faces(image_pil, confidence_threshold=0.7, threshold=0.4):
    ...
    for i, data in enumerate(faces):
        ...
        # שימוש במודל שלנו לחישוב embedding
        emb = get_embedding(face_pil)
